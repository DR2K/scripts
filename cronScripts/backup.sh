...
DELETE_METHOD="sequential" # Choices: thin, sequential, none; sequential: delete oldest; thin: keep last 24 hourly, last 30 daily, and monthly (use with 1 hr cron interval)
...
WINDOW_MANAGER="tmux" # Choices: screen, tmux

# Other Variables (do not modify)
DATE_FORMAT="%F_%H-%M-%S"
TIMESTAMP=$(date +$DATE_FORMAT)

while getopts 'cd:f:hi:m:o:p:qs:vw:' FLAG; do
  case $FLAG in
    c) ENABLE_CHAT_MESSAGES=true ;;
    d) DELETE_METHOD=$OPTARG ;;
    f) TIMESTAMP=$OPTARG ;;
    h) echo "Minecraft Backup (by Nicolas Chan, modified by Devdeep Rajpal)"
  ...
  if [[ $SCREEN_NAME != "" ]]; then
    case $WINDOW_MANAGER in
      "screen") screen -S $SCREEN_NAME -p 0 -X stuff "$COMMAND$(printf \\r)"
        ;;
      "tmux") tmux -S "/var/tmux_sessions/$SCREEN_NAME" send-keys "$COMMAND" ENTER
        ;;
    esac
  fi
}
...
[ -d $BACKUP_DIRECTORY ] && dirCheck=1 || dirCheck=0

if [[ $dirCheck == 0 ]]; then
  echo "initial"
  mkdir $BACKUP_DIRECTORY
  mkdir $BACKUP_DIRECTORY/model/
  rsync -av $SERVER_WORLD $BACKUP_DIRECTORY/model/
fi
if [[ $dirCheck == 1  ]]; then
  echo "subsequent"
  shopt -s lastpipe
  latest=$(ls -td -- $BACKUP_DIRECTORY/*/ | head -n 2 | cut -d':' -f1)
  rsync -av --delete $SERVER_WORLD --link-dest $latest --exclude="logs/" --exclude=".mixin.out/" $ARCHIVE_PATH
fi
...
WORLD_SIZE_BYTES=$(du -bs $SERVER_WORLD | awk '{print $1}')
ARCHIVE_SIZE_BYTES=$(du -bs $ARCHIVE_PATH | awk '{print $1}')
ARCHIVE_SIZE=$(du -hs $ARCHIVE_PATH | awk '{print $1}')
BACKUP_DIRECTORY_SIZE=$(du -hs $BACKUP_DIRECTORY | awk '{print $1}')
TIME_DELTA=$((END_TIME - START_TIME))

message-players-success "Backup complete!" "$TIME_DELTA s, $ARCHIVE_SIZE/$BACKUP_DIRECTORY_SIZE"
delete-old-backups
